...
Per-gig web platform for DJs. A monorepo with three packages: `server/` (Express + MongoDB API), `client/` (public attendee landing page), and `admin/` (private DJ dashboard). The full spec lives in `docs/DJ_GIG_PLATFORM_SPEC.md` — read it before implementing anything.
